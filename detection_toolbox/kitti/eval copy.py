...
def fused_compute_statistics(overlaps,
    ...
    for i in range(gt_nums.shape[0]):
        ...
        for t, thresh in enumerate(thresholds):
            ...
            tp, fp, fn, similarity, _, extra = compute_statistics_jit(
                overlap,
                gt_data,
                dt_data,
                ignored_gt,
            ...
            gt_box_type, dt_box_type, gt_box_matched_idx, dt_box_matched_idx = extra

            extras['gt_box_types'][t].append(gt_box_type)
            extras['dt_box_types'][t].append(dt_box_type)
            extras['gt_box_matched_idxs'][t].append(gt_box_matched_idx)
            extras['dt_box_matched_idxs'][t].append(dt_box_matched_idx)
...
def eval_class(gt_annos,
    ...
    extrass = {
        ...
        "gt_box_matched_idxss": np.full((num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS), None, dtype=object),
        "dt_box_matched_idxss": np.full((num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS), None, dtype=object)
    }
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                extras = {
                    ...
                    "gt_box_matched_idxs": np.empty(N_SAMPLE_PTS, dtype=object),
                    "dt_box_matched_idxs": np.empty(N_SAMPLE_PTS, dtype=object)
                }
                extras['gt_box_types'][...] = [[] for _ in range(N_SAMPLE_PTS)]
                extras['dt_box_types'][...] = [[] for _ in range(N_SAMPLE_PTS)]
                extras['gt_box_matched_idxs'][...] = [[] for _ in range(N_SAMPLE_PTS)]
                extras['dt_box_matched_idxs'][...] = [[] for _ in range(N_SAMPLE_PTS)]
                ...
                extrass['gt_box_matched_idxss'][m, l, k, :] = extras['gt_box_matched_idxs']
                extrass['dt_box_matched_idxss'][m, l, k, :] = extras['dt_box_matched_idxs']
...
def do_eval_v3(gt_annos,
               ...
               z_center=1.0,
               extra_info=None,
               eval_modes=[0, 1, 2]): #! Represents idxes of types to evaluate (here, it's 0 for bbox, 1 for bev, 2 for 3d) so all 3
    # min_overlaps: [num_minoverlap, metric, num_class]
    types = ["bbox", "bev", "3d"]
    metrics = {}
    for i in eval_modes:
...
def kitti_eval(
    ...
    ... = {
        ...
        "gt_box_matched_idx": gt_box_matched_idx, # same shape as above, holds either -1 or idx of matched dt box
